...
async function fetchData(reciter, album, track, lyric) {
  await Promise.all([
    ...
    store.dispatch('lyrics/fetchLyric', {
      reciter, album, track, lyric,
    }),
  ]);
}
...
export default {
  ...
  methods: {
    async uploadForm() {
      ...
      await store.dispatch('lyrics/updateLyric', {
        reciter: this.reciter.slug, album: this.album.year, track: this.track.slug, lyric: this.lyric.id, form,
      });
      this.$router.push({
        name: 'Track-Page',
